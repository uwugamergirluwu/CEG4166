...
from PlotDataRobot_Lab3 import multiplePlots
...
import termios
import time
from StudentNumberFlash import StudentNumberMonitorFlasher
...
def avoidance():
    ...
    left_distances = sonarSensor.sweep(5, "left")
    right_distances = sonarSensor.sweep(5, "right")
...
def keyboard():
    ...
    detection_process = None  # This will hold the process for stopping it later

    # Parameters for object detection (update as needed)
    model_dir = "ssdmodel"  # Model directory for the detection
    ...
    # Capture keyboard input
        ...
        if char == "w":
            ...
            if not detection_running:
                object_detection.startObjectDetection()
            ...
            sonarSensor.move_servo(1400)

        elif char == 'l':
            sonarSensor.move_servo(2400)

        elif char == 'j':
            sonarSensor.move_servo(700)
...
def loopData(self):
    plotData.updateData()
    return plotData.p011, plotData.p012, plotData.p021, plotData.p022, plotData.p031, plotData.p041
...
xmax = 5
plotData = multiplePlots(leftEncoderCount, rightEncoderCount, sonarSensor, samples, xmax)

student_numbers = ["300234999", "300241133", "300240028"]
monitor_flasher = StudentNumberMonitorFlasher(student_numbers)


###Thread instantiation
sonarThread = threading.Thread(target=Sonar, args=(sonarSensor, samples))
keyboardThread = threading.Thread(target=keyboard, args=())
studentNumFlash = threading.Thread(target=monitor_flasher.run(), args=())

simulation = animation.FuncAnimation(fig=plotData.f0, func=loopData,
                   blit=False, frames=200, interval=20, repeat=False)


studentNumFlash.start()
