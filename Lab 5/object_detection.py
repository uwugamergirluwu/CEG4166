...
parser.add_argument('--modeldir', default='/home/pi/Desktop/CEG4166/Lab3/ssdmodel')
...
video_stream = Video_PiCamera(resolution=(imW, imH), framerate=60)
time.sleep(1)
...
def detection():
    while True:
        ...
        frame_rgb = cv2.cvtColor(original_frame, cv2.COLOR_RGB2BGR)  # Convert RGB to BGR for OpenCV
        ...
        for i in range(len(conf_values)):
            if minimum_confidence <= conf_values[i] <= 1.0:
                ...
                cv2.rectangle(frame_rgb, (xmin, label_ymin - label_size[1] - 10), 
